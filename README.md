...
They are generated by `tools/build_workflows.py`, which reads every node's real
input list from a running ComfyUI's `/object_info` rather than hand-writing it.
If you change the graphs, change that script and re-run it.
